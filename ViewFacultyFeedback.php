...
$sql = "SELECT * FROM facultyfeedback";
$result = $conn->query($sql);
	...
	if ($result->num_rows > 0) {
			...
			//while($row = $result->fetch_assoc()) {
				$i = 1;
				echo "<table align='center' border=2px><tr bgcolor='aqua'><th>SNo</th><th align='center' width='150'>Feedback</th><th>Interaction with class</th><th>Explanation with example</th><th>Doubt clearence</th></tr>";
					// output data of each row
					while($row = $result->fetch_assoc()) {
						echo "<tr align='center' bgcolor='white'><td>".$i."</td><td  align='left' width='150'>" . $row["feedback"]. "</td><td>" . $row["interaction"]."</td><td>" . $row["example"]."</td><td>" . $row["doubt"]. "</td></tr>";
						$i++;
					}
					echo "</table>";
		} else {
